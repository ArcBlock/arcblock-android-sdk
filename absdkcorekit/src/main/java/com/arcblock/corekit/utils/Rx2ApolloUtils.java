...
public class Rx2ApolloUtils {
	...
	@NotNull
	public static <T> Flowable<Response<T>> fromWithFlowable(@NotNull ApolloCall<T> call) {
		return fromWithFlowable(call, BackpressureStrategy.LATEST);
	}

	@NotNull
	public static <T> Flowable<Response<T>> fromWithFlowable(@NotNull final ApolloCall<T> call,
			...
			public void subscribe(final FlowableEmitter<Response<T>> emitter) throws Exception {
				...
				call.enqueue(
						new ApolloCall.Callback<T>() {
							...
							@Override
							public void onStatusEvent(@NotNull ApolloCall.StatusEvent event) {
								if (event == ApolloCall.StatusEvent.COMPLETED && !emitter.isCancelled()) {
									emitter.onComplete();
								}
							}
						}
